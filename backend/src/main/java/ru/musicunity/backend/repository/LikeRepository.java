...
public interface LikeRepository extends JpaRepository<Like, Long> {
    ...
    @Query("SELECT DISTINCT l.review FROM Like l WHERE l.type = :type ORDER BY l.review.createdAt DESC")
    Page<Review> findAllReviewsByLikeType(@Param("type") LikeType type, Pageable pageable);
    
    @Query("SELECT DISTINCT r FROM Review r JOIN FETCH r.user JOIN FETCH r.release JOIN Like l ON l.review = r WHERE l.type = :type ORDER BY r.createdAt DESC")
