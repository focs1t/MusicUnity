...
function ReleasePage() {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      console.log('СОЗДАНИЕ РЕЦЕНЗИИ: Рецензия успешно создана, устанавливаем флаг userHasReviewed = true');
      console.log('СОЗДАНИЕ РЕЦЕНЗИИ: Сейчас userHasReviewed =', userHasReviewed);
      // Принудительно задержим выполнение, чтобы успело обновиться состояние
      setTimeout(() => {
        console.log('СОЗДАНИЕ РЕЦЕНЗИИ: После задержки userHasReviewed =', userHasReviewed);
      }, 500);
  ...
  // Эффект для проверки наличия рецензии (отдельно)
  useEffect(() => {
    const checkUserReviewStatus = async () => {
      if (!user || !id) return;
      
      console.log('ОТДЕЛЬНАЯ ПРОВЕРКА: Выполняется проверка наличия рецензии');
      
      try {
        // Проверяем напрямую через получение всех рецензий
        const hasReviewDirectly = await checkUserReviewDirectly();
        console.log(`ОТДЕЛЬНАЯ ПРОВЕРКА: Результат прямой проверки: ${hasReviewDirectly}`);
        
        setUserHasReviewed(hasReviewDirectly);
      } catch (err) {
        console.error('ОТДЕЛЬНАЯ ПРОВЕРКА: Ошибка при проверке наличия рецензии', err);
      }
    };
  ...
  // Новая функция для проверки наличия рецензии пользователя среди всех рецензий релиза
  const checkUserReviewDirectly = async () => {
    try {
      const currentUserId = getCurrentUserIdInComponent();
      if (!currentUserId || !id) {
        console.log('КРИТИЧЕСКАЯ ПРОВЕРКА: Нет данных для проверки рецензии (ID пользователя или ID релиза)');
        return false;
      }
      
      console.log(`КРИТИЧЕСКАЯ ПРОВЕРКА: Прямая проверка наличия рецензии для пользователя ${currentUserId} на релиз ${id}`);
      
      // Получаем все рецензии для данного релиза
      const allReviews = await reviewApi.getExtendedReviewsByRelease(id, 0, 100);
      console.log('КРИТИЧЕСКАЯ ПРОВЕРКА: Получено рецензий:', allReviews?.content?.length || 0);
      ...
      if (allReviews && allReviews.content && allReviews.content.length > 0) {
        // Логируем все ID авторов рецензий для отладки
        console.log('КРИТИЧЕСКАЯ ПРОВЕРКА: Все ID авторов рецензий:', 
          allReviews.content.map(r => r.user?.userId || r.user?.id));
        ...
        if (userReview) {
          console.log(`КРИТИЧЕСКАЯ ПРОВЕРКА: НАЙДЕНА рецензия пользователя:`, userReview);
        } else {
          console.log(`КРИТИЧЕСКАЯ ПРОВЕРКА: Рецензия пользователя НЕ НАЙДЕНА`);
        }
        
        return !!userReview;
      }
      
      console.log('КРИТИЧЕСКАЯ ПРОВЕРКА: Рецензий для проверки не найдено');
      return false;
    } catch (error) {
      console.error('КРИТИЧЕСКАЯ ПРОВЕРКА: Ошибка при прямой проверке наличия рецензии:', error);
      return false;
    }
  };
  ...
  useEffect(() => {
    const fetchRelease = async () => {
      try {
        ...
        const currentUserIdInUserEffect = user?.id || getCurrentUserId();
        console.log('ПРОВЕРКА РЕЦЕНЗИИ: currentUserIdInUserEffect =', currentUserIdInUserEffect);
        console.log('ПРОВЕРКА РЕЦЕНЗИИ: id релиза =', id);
        console.log('ПРОВЕРКА РЕЦЕНЗИИ: текущее значение userHasReviewed =', userHasReviewed);
        
        if (currentUserIdInUserEffect) {
          try {
            console.log(`ПРОВЕРКА РЕЦЕНЗИИ: Проверяем наличие рецензии с использованием прямого метода проверки`);
            // Используем новый метод прямой проверки через получение всех рецензий
            const hasReviewedDirectly = await checkUserReviewDirectly();
            console.log(`ПРОВЕРКА РЕЦЕНЗИИ: Результат прямой проверки: ${hasReviewedDirectly ? 'РЕЦЕНЗИЯ НАЙДЕНА' : 'РЕЦЕНЗИЯ НЕ НАЙДЕНА'}`);
            
            // Если первая проверка не дала результатов, пробуем альтернативный метод
            if (!hasReviewedDirectly) {
              console.log('ПРОВЕРКА РЕЦЕНЗИИ: Первая проверка не нашла рецензию, проверяем еще раз через API');
              try {
                const hasReviewApi = await reviewApi.hasUserReviewedRelease(currentUserIdInUserEffect, id);
                console.log(`ПРОВЕРКА РЕЦЕНЗИИ: Результат проверки через API: ${hasReviewApi ? 'РЕЦЕНЗИЯ НАЙДЕНА' : 'РЕЦЕНЗИЯ НЕ НАЙДЕНА'}`);
                
                // Используем результат второй проверки
                if (hasReviewApi !== hasReviewedDirectly) {
                  console.log(`ПРОВЕРКА РЕЦЕНЗИИ: Результаты проверок различаются, используем результат API проверки`);
                  setUserHasReviewed(hasReviewApi);
                  return;
                }
              } catch (apiError) {
                console.error('ПРОВЕРКА РЕЦЕНЗИИ: Ошибка при альтернативной проверке:', apiError);
              }
            }
            
            console.log(`ПРОВЕРКА РЕЦЕНЗИИ: Устанавливаем userHasReviewed = ${hasReviewedDirectly}`);
            setUserHasReviewed(hasReviewedDirectly);
            console.log(`ПРОВЕРКА РЕЦЕНЗИИ: После установки состояния userHasReviewed = ${userHasReviewed} (может отображаться старое значение)`);
          } catch (reviewCheckError) {
            console.error('ПРОВЕРКА РЕЦЕНЗИИ: Ошибка при проверке наличия рецензии:', reviewCheckError);
            // В случае ошибки предполагаем, что рецензии нет
            console.log(`ПРОВЕРКА РЕЦЕНЗИИ: Из-за ошибки устанавливаем userHasReviewed = false`);
            setUserHasReviewed(false);
          }
        } else {
          console.log('ПРОВЕРКА РЕЦЕНЗИИ: Не удалось получить ID пользователя для проверки наличия рецензии');
        }
      ...
      <div className="site-content">
        <main className="main-container">
          <div className="container">
            ...
            {console.log('БЛОК РЕЦЕНЗИИ: Проверяем условие для сообщения о наличии рецензии')}
            {console.log('БЛОК РЕЦЕНЗИИ: user =', !!user)}
            {console.log('БЛОК РЕЦЕНЗИИ: !isAuthor =', !isAuthor)}
            {console.log('БЛОК РЕЦЕНЗИИ: userHasReviewed =', userHasReviewed)}
            {console.log('БЛОК РЕЦЕНЗИИ: Итоговое условие =', user && !isAuthor && userHasReviewed)}
            ...
            {user && (
              <div style={{ 
                ...
                ОТЛАДКА:<br/>
                Пользователь: {user?.id || 'нет ID'}<br/>
                Автор: {isAuthor ? 'да' : 'нет'}<br/> 
                Есть рецензия: {userHasReviewed ? 'да' : 'нет'}
                <button 
                  onClick={async () => {
                    const hasReview = await checkUserReviewDirectly();
                    console.log('ПРОВЕРКА ВРУЧНУЮ:', hasReview);
