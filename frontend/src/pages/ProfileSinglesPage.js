...
/**
 * Страница с синглами и EP пользователя
 */
function ProfileSinglesPage() {
  ...
  /**
   * Загрузка синглов и EP пользователя
   */
  useEffect(() => {
    const fetchReleases = async () => {
      try {
        ...
        // Фильтруем только синглы и EP
        const singleEpReleases = response.content.filter(release => {
          const releaseTypeUpper = release.type?.toUpperCase();
          return releaseTypeUpper === 'SINGLE' || releaseTypeUpper === 'EP';
        });
        
        setReleases(singleEpReleases);
        setTotalPages(response.totalPages);
        setTotalElements(singleEpReleases.length);
      } catch (err) {
        console.error('Ошибка при загрузке синглов и EP:', err);
        setError('Не удалось загрузить синглы и EP. Пожалуйста, попробуйте позже.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handlePageChange = (newPage) => {
    ...
    navigate(`/profile/${userId}/singles?page=${newPage + 1}`); // URL использует 1-based индекс
    window.scrollTo(0, 0);
  };
      ...
      pages.push(
        React.createElement('li', { key: 'prev' }, 
          React.createElement('a', {
            href: `/profile/${userId}/singles?page=${currentPage}`,
    ...
    if (startPage > 0) {
      pages.push(
        React.createElement('li', { key: 1 }, 
          React.createElement('a', {
            href: `/profile/${userId}/singles?page=1`,
    ...
    for (let i = startPage; i <= endPage; i++) {
      pages.push(
        React.createElement('li', { key: i + 1 }, 
          React.createElement('a', {
            href: `/profile/${userId}/singles?page=${i + 1}`,
    ...
    if (endPage < totalPages - 1) {
      pages.push(
        React.createElement('li', { key: totalPages }, 
          React.createElement('a', {
            href: `/profile/${userId}/singles?page=${totalPages}`,
            className: 'pagination-button',
            onClick: (e) => {
              e.preventDefault();
    ...
    if (currentPage < totalPages - 1) {
      pages.push(
        React.createElement('li', { key: 'next' }, 
          React.createElement('a', {
            href: `/profile/${userId}/singles?page=${currentPage + 2}`,
    ...
    React.createElement('main', {}, [
      React.createElement('div', { key: 'header', className: 'container' }, 
        React.createElement('h1', { key: 'title', className: 'text-lg md:text-xl lg:text-3xl font-bold mb-4 lg:mb-8' }, 
          `Синглы и EP${userDetails?.username ? ` пользователя ${userDetails.username}` : ''}`
        )
      ),
      ...
      React.createElement('div', { key: 'content', className: 'container mt-5' },
        React.createElement('div', {},
          React.createElement('div', { className: 'releases-grid' },
            ...
            releases.length === 0 ?
              React.createElement('div', { className: 'col-span-full text-center py-8' }, 'Нет избранных синглов и EP') :
...
export default ProfileSinglesPage; 